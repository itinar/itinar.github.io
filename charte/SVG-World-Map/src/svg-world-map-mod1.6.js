...
var svgWorldMap = (function() { 
            ...
            svg.addEventListener("load", async () => {
                let promise2 = new Promise(resolve2 => {
                    ...
                    svgMap = { 
                        ...
                        'updateTest': function(data) { window["updateMapDataTest"].call(null, data); },
                        'updateRaw': function(data) { window["updateMapDataRaw"].call(null, data); },

                        'getElementByIdInsider': function(data) {window["getElementByIdInsider"].call(null, data); },

                        'pathSetAttributesExternal': function(data) { window["pathSetAttributesExternal"].call(null, data); },
                        'pathSetAttributesRaw': function(data) { window["pathSetAttributesRaw"].call(null, data); },

                        //'findIdByName': function(id){window["findIdByName"].call(null, id);}
                    };
                    // Load time controls
    ...
    function initMap() {
        ...
        if (document.getElementById('svg-world-map-container') == null) {
            ...
            // Load big map with provinces
                ...
                svg.setAttribute("data", options.libPath + "world-states-provinces-stripes-test04.svg");
            }
    ...
    function initMapCountries() {
        ...
        baseNode.childNodes.forEach(function(node) {
            ...
            console.log("init: countries" + countries + " countries.id = node");
    ...
    function pathSetAttributes(path, event) {
        //console.log("pathSetAttributes");
        //console.log("path");console.log(path);
        //console.log("event");console.log(event);
        if (path != undefined && path.id != 'World' && path.id != 'Ocean') {
            //console.log("1: path:" + path + " path.id: " + path.id);
            ...
            if (event == 'click' || ((event == 'out' || event == 'over') /*&& path != selectedCountry && path.country != selectedCountry*/)) {
                ...
                ////console.log("selectedCountry");////console.log(selectedCountry);
                ////console.log("path.country");////console.log(path.country);
                ////console.log("2");

                if (path == path.country.border || path.parentNode == path.country.border) {
                    //console.log("3: path.country.border:" + path.country.border + " path.parentNode: " + path.parentNode + " path.country.border: " + path.country.border);
                    path.setAttribute('stroke', options.countryStroke[event]);
                    path.setAttribute('stroke-width', options.countryStrokeWidth[event]);
                // Other provinces
                } else {
                    //console.log("path.updateColor: " + path.updateColor);
                    // Keep updated color 
                    if (path.updateColor != undefined) {
                        ////console.log("5");
                        path.setAttribute('fill', path.updateColor);
                        ////console.log("path.updateColor");////console.log(path.updateColor);
                    } else {
                        ////console.log("6");
                        path.setAttribute('fill', options.provinceFill[event]);
                        ////console.log("options.provinceFill[event]");////console.log(options.provinceFill[event]);
                    }
    ...
    window.updateMapData = function(updateData) {
        ...
        console.log("1: updateData " + updateData);
        for (var id in updateData) {
            console.log("2: id " + id);
            if (countries[id] != undefined) {
                var country = countries[id];
                var cons = "country  [id] -> " + country;
                console.log("3.1: country[id]: " + country);
            } else {
                var country = findProvinceById(id);
                var cons = "country  findProvinceById -> " + country;
                console.log("3.2: findProvinceById: " + country);
            }
            if (country != undefined) {
                cons += "// country != undefined";
                cons += "// country.provinces -> " + country.provinces;
                if (country.provinces == undefined) { // Is mostly a province and no country. TODO: Rename variables?
                    cons += "// country-provinces == undefined"; 
                    console.log("4.1: country.updateColor: " + country.updateColor + " // updateData[id]: " + updateData[id]);
                    ...
                    country.provinces.forEach(function(province) { 
                        ...
                        console.log("4.2.1: province: " + province + "province.updateColor: " + province.updateColor + " // updateData[id]: " + updateData[id]);
                        if (province.provinces != undefined) {
                            province.provinces.forEach(function(subprovince) { 
                                subprovince.updateColor = updateData[id];
                                console.log("4.2.2: subprovince: " + subprovince + " subprovince.updateColor: " + subprovince.updateColor + " // updateData[id]: " + updateData[id]);
        ...
        // Update function for calling home from the outside, defined in 'svgMap.update' 
    window.updateMapDataTest = function(updateData) {
        //console.log("updateMapData");
        console.log("1: updateData " + updateData);
        for (var id in updateData) {
            console.log("2: id " + id);
            ...
            if (country != undefined) {
                ...
                if (country.provinces == undefined) { // Is mostly a province and no country. TODO: Rename variables?
                    ...
                    console.log("4.1: country.updateColor: " + country.updateColor + " // updateData[id]: " + updateData[id]);
                    country.updateColor = updateData[id];
                    myWorldMap.pathSetAttributesExternal(country, updateData[id]);
                } else {
                    /*country.updateColor = updateData[id];
                    pathSetAttributes(country, updateData[id]);*/ 
                    country.provinces.forEach(function(province) { 
                        province.updateColor = updateData[id];
                        myWorldMap.pathSetAttributesExternal(province, updateData[id]);
                        console.log("4.2.1: province: " + province + "province.updateColor: " + province.updateColor + " // updateData[id]: " + updateData[id]);
                        if (province.provinces != undefined) {
                            province.provinces.forEach(function(subprovince) { 
                                subprovince.updateColor = updateData[id];
                                console.log("4.2.2: subprovince: " + subprovince + " subprovince.updateColor: " + subprovince.updateColor + " // updateData[id]: " + updateData[id]);
                                myWorldMap.pathSetAttributesExternal(subprovince, updateData[id]);
                            });
                        }
                    });
                }
    ...
    window.pathSetAttributesExternal = function(path, event) {
        //console.log("pathSetAttributes");
        console.log("path");console.log(path);
        console.log("event");console.log(event);
        if (path != undefined && path.id != 'World' && path.id != 'Ocean') {
    ...
    window.updateMapDataRaw = function(updateData) {
        //console.log("updateMapData");
        console.log("1: updateData " + updateData);
        for (var id in updateData) {
            console.log("2: id " + id);
    ...
    window.pathSetAttributesRaw = function(path, color) {
        document.getElementById("fr").setAttribute("fill", "red");
    }


    window.getElementByIdInsider = function(id) {
    ...
    function buildCountryGroups() {
        //console.log("callbuildCountryGroupsBack");
        for (var country in countries) {
