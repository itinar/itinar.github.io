...
var svgWorldMap = (function() { 
    ...
    var options = {
        ...
        countryStrokeWidth: { out: '0.2',  over: '0.4',  click: '0.4' }, 
        provinceFill: { out: '#000000',  over: '#000000',  click: '#000000' }, 
        provinceStroke: { out: '#FFFFFF',  over: '#FFFFFF',  click: '#666666' }, 
        provinceStrokeWidth: { out: '0',  over: '0.2',  click: '0.2' }, 
        // Group options
        ...
        let promise1 = new Promise(resolve1 => {
            ...
            svg.addEventListener("load", async () => {
                let promise2 = new Promise(resolve2 => {
                    ...
                    //console.log("svg.getSVGDocument().children[0]");//console.log(svg.getSVGDocument().children[0]);
                    //console.log("svg.getSVGDocument().children[1]");//console.log(svg.getSVGDocument().children[1]);
                    //console.log("svg.getSVGDocument().children[2]");//console.log(svg.getSVGDocument().children[2]);
                    //console.log("svg.getSVGDocument().children[3]");//console.log(svg.getSVGDocument().children[3]);
                    ...
                    svgMap = { 
                        ...
                        'infoBox': function(event, data, stateData) { window["updateInfoBoxTest"].call(null, event, data, stateData); }
                    };
                    // Load time controls
    ...
    function initMap() {
        ...
        if (document.getElementById('svg-world-map-container') == null) {
            ...
            svg.setAttribute("data", options.libPath + "world-states-provinces-stripes-test05-smaller-color.svg");
            ...
            style.innerHTML = `#svg-world-map { width: 100%; height: 100%; }`;
    ...
    function sortLabels() {
        countries['labels'].childNodes.forEach(function(label) { 
            ...
            if (label.tagName == 'text') {
                ...
                label.addEventListener("mouseclick", function() { countryClick(this.id.substr(0, 2)); });
                //label.addEventListener("touchend", function() { countryClick(this.id.substr(0, 2)); });
            }
        });
    ...
    window.updateInfoBoxTest = function(event, text, stateData) {
    ...
    function initMapControls() {
        for (var country in countries) {
            ...
            countries[country].addEventListener("touchend", function() { provinceClick(); });*/
            for (var i = countries[country].children.length; i > 0; i--) {
                //console.log("i: " + i);
                countries[country].children[i-1].addEventListener("mouseover", function() { provinceOverOut(this, 'over'); });
                countries[country].children[i-1].addEventListener("mouseout", function() { provinceOverOut(this, 'out'); });
                countries[country].children[i-1].addEventListener("mouseup", function() { provinceClick(this); });
                ////console.log("countries[country].children[i-1]");//console.log(countries[country].children[i-1]);
            }
        }
    }

    // Map country hover handling
    function provinceOverOut(id, overout) {
        var province = id; // Get (sub-)country / province / state
